...
import type { LadderLift } from './ladderProgram';
import { makeLadderLift, resolveLadderDay } from './ladderProgram';
 ...
 * calentamiento -> serie de trabajo dentro de la misma sesion. Esa estructura ("día = varios lifts,
 * cada uno con escalera de %") vive en `ladderProgram.ts`; aquí solo están los datos de las 14
 * semanas y la resolución de en qué día del ciclo está hoy el atleta.
 */

type HalteroDay = LadderLift[];
type HalteroWeek = HalteroDay[];

const snatch = (spec: string, isMaxAttempt?: boolean) => makeLadderLift('Snatch', 'snatch', 'snatch', 'oly', spec, { isMaxAttempt });
const powerSnatch = (spec: string) => makeLadderLift('Power Snatch', 'power-snatch', 'snatch', 'oly', spec);
const hangSnatch = (spec: string, label = 'Hang Power Snatch') => makeLadderLift(label, 'hang-snatch', 'snatch', 'oly', spec);
const snatchPull = (spec: string, label = 'Tirón Snatch') => makeLadderLift(label, 'snatch-pull', 'snatch', 'oly', spec);
const clean = (spec: string) => makeLadderLift('Clean', 'clean', 'clean', 'oly', spec);
const powerClean = (spec: string) => makeLadderLift('Power Clean', 'power-clean', 'clean', 'oly', spec);
const hangClean = (spec: string) => makeLadderLift('Hang Power Clean', 'hang-clean', 'clean', 'oly', spec);
const cleanPull = (spec: string, label = 'Tirón de Clean') => makeLadderLift(label, 'clean-pull', 'clean', 'oly', spec);
const cleanJerk = (spec: string, isMaxAttempt?: boolean) => makeLadderLift('Clean & Jerk', 'clean-and-jerk', 'cleanAndJerk', 'oly', spec, { isMaxAttempt });
const pushJerk = (spec: string) => makeLadderLift('Push Jerk', 'push-jerk', 'cleanAndJerk', 'oly', spec);
const splitJerk = (spec: string) => makeLadderLift('Split Jerk', 'split-jerk', 'cleanAndJerk', 'oly', spec);
const backSquat = (spec: string, isMaxAttempt?: boolean) => makeLadderLift('Back Squat', 'back-squat', 'backSquat', 'strength', spec, { isMaxAttempt });
const frontSquat = (spec: string) => makeLadderLift('Front Squat', 'front-squat', 'frontSquat', 'strength', spec);
...
export function resolveHalteroDay(
  ...
  // El motor de escaleras hace el trabajo comun (autorregulacion salvo max, redondeo a disco, nota de
  // escalera); aqui solo se le pone el prefijo de ciclo/semana al `format` y la nota de semana dura.
  const lifts = resolveLadderDay(day, prs, autoregFactor).map((lift) => ({
